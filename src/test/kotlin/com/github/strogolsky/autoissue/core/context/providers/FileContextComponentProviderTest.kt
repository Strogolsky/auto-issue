...
class FileContextComponentProviderTest {
    ...
    @Test
    fun should_ReturnComponent_When_ContextExtractedSuccessfully() = runBlocking {
        // --- TEST FLOW ---
        // 1. ARRANGE
        val classContext = mockk<ClassContext> {
            every { name } returns "UserService"
            every { fields } returns listOf("private val repository")
        }
        val methodContext = mockk<MethodContext> {
            every { signature } returns "fun getUser()"
            every { body } returns "return repository.getUser()"
        }
        val detailedContext = mockk<DetailedFileContext> {
    ...
    @Test
    fun should_ReturnPartialComponent_When_ClassAndMethodAreNull() = runBlocking {
        // --- TEST FLOW ---
        // 1. ARRANGE
        val detailedContext = mockk<DetailedFileContext> {
    ...
    @Test
    fun should_ReturnNull_When_ContextExtractionFails() = runBlocking {
        // --- TEST FLOW ---
        // 1. ARRANGE
